...
REM .bat con permisos de administrador - Modern Windows 11 Admin privileges
:-------------------------------------
net session >nul 2>&1
if '%errorlevel%' NEQ '0' (
...
echo.
set "file="
set "pscmd=Add-Type -AssemblyName System.Windows.Forms; $f = New-Object System.Windows.Forms.OpenFileDialog; $f.Filter = 'ROM Archive (*.zip)|*.zip|All Files (*.*)|*.*'; if($f.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK){$f.FileName}"
for /f "usebackq delims=" %%p in (`powershell -NoProfile -ExecutionPolicy Bypass -Command "!pscmd!"`) do set "file=%%p"

if not defined file (
    echo No file selected or operation canceled.
